...
os.environ["TRANSFORMERS_CACHE"] = "/home/ubuntu/nanogpt4crate/arthursae/.cache" # Cache transformers
...
lm = transformer_lens.HookedTransformer.from_pretrained(cfg['model_name'], fold_ln=False).to(cfg["dtype"])
...
# dataset_path = "/home/ubuntu/nanogpt4crate/data/pile/test.bin"

if cfg['dataset_split'] == 'test':
    dataset_path = "/home/ubuntu/nanogpt4crate/data/pile/test.bin"
    raw_all_data = np.memmap(os.path.join(dataset_path), dtype=np.uint16, mode='r')
    stacked_raw_data = raw_all_data[:raw_all_data.shape[0]//3072 * 3072].reshape(-1, 1024)
    print(type(stacked_raw_data))
    stacked_torch_data = torch.from_numpy(stacked_raw_data.astype(np.int64))
    print("before replicate shape", stacked_torch_data.shape)
    stacked_torch_data = stacked_torch_data.repeat(10, 1)
    print("after replicate shape", stacked_torch_data.shape)
    raw_all_data = iter(stacked_torch_data)
elif cfg['dataset_split'] == 'train':
    dataset_path = "/home/ubuntu/nanogpt4crate/data/pile/train.pt"
    stacked_torch_data = torch.load(dataset_path).reshape(-1, 1024)
    print("stacked_torch_data shape", stacked_torch_data.shape)
    raw_all_data = iter(stacked_torch_data)
...
run_name = f'MODEL-{cfg["model_name"]}-LAYER-{cfg["layer"]}-MULT-{cfg["d_sae"] // cfg["d_in"]}x-LR-{cfg["lr"]}-LAMBDA-{cfg["l1_lambda"]}-DSAW-{cfg["d_sae"]}{ctime().replace(" ", "_").replace(":", "-") + "_" + str(randint(1, 100))}'

os.environ["WANDB_API_KEY"] = "f97d2d399febebcd6615237f004e6b14474595b1"
if True:
    wandb.init(
        project="sae",
...
save_path = f"../weights/{cfg['model_name']}-{cfg['d_sae'] // cfg['d_in']}x/{cfg['layer']}"
if not os.path.exists(save_path):
    os.makedirs(save_path, exist_ok=True)
# assert os.path.exists(f"../weights/{cfg['model_name']}/{cfg['layer']}") # For weight saving
...
if True: # Usually we don't want to profile, so `if True` is better as it keeps things in scope
    ...
    for step_idx in step_iterator:
        ...
        if step_idx == 0:
            ...
            def no_change_hook(mlp_post, hook):
                mlp_post[:] = mlp_post[:]
                return mlp_post
            
            def replacement_hook(mlp_post, hook, encoder):
                mlp_post[:] = encoder(mlp_post, return_mode="sae_out")
                # print("mlp_post shape", mlp_post.shape, "mlp_post_reconstr shape", mlp_post_reconstr.shape)
                return mlp_post
            ...
            #     print("Prelosses", neglogprobs.item())
            #     prelosses.append(neglogprobs.item())
            model_loss = sae.get_test_loss(lm=lm, test_tokens=test_tokens, cfg=cfg, hook = no_change_hook).item()
            # wandb.log({"model_loss": model_loss})
            prelosses.append(model_loss)
            zero_loss = sae.get_test_loss(lm=lm, test_tokens=test_tokens, cfg=cfg, hook = zero_ablate_hook).item()
            # wandb.log({"zero_loss": zero_loss})
            prelosses.append(zero_loss)
        ...
        if step_idx%cfg["test_every"]==0:
            ...
            metrics["test_loss_with_sae"] = sae.get_test_loss(lm=lm, test_tokens=test_tokens, cfg=cfg, hook = partial(replacement_hook, encoder=sae)).item()
            # wandb.log({"reconstr loss": metrics["test_loss_with_sae"]})
            metrics["loss_recovered"] = (zero_loss-metrics["test_loss_with_sae"])/(zero_loss-model_loss)
        ...
        if cfg["save_state_dict_every"](step_idx) or save_weights_failed:
            ...
            if cfg["delete_cache"]:
                try:
                    ...
                    subprocess.run(f"rm -rf {save_path}/**.pt", shell=True)
