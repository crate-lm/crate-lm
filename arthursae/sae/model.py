...
class SAE(HookedRootModule):
    ...
    def forward(
        ...
        if activate_feature is not None:
            hidden_post[:, :, activate_feature] = 10.0
    ...
    def anthropic_resample(
        ...
        print("Doing anthropic resampling")
        ...
        for refill_batch_idx_start in anthropic_iterator:
            ...
            sae_loss = self.get_test_loss(
                ...
                return_mode="all",
                cfg=self.cfg,
                hook = partial(replacement_hook, encoder=self.forward),
            )
    ...
    def load_from_local(
        ...
        state_dict = torch.load(path, **({"map_location": torch.device('cpu')} if not torch.cuda.is_available() else {}))
        self.load_state_dict(state_dict=state_dict)
