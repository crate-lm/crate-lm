...
def get_batch_tokens(
    ...
    while batch_tokens.shape[0] < batch_size:
        ...
        # the input next(dataset) is already a numpy vector
        tokens = next(dataset).to(device or lm.cfg.device)
        # print(tokens)
...
def get_cfg(**kwargs) -> Dict[str, Any]: # TODO remove Any
    cur_dict = {
        ...
        "d_in": 128,  # Input dimension for the encoder model
        "d_sae": 128 * 16,  # Dimensionality for the sparse autoencoder (SAE)
        "lr": 4e-4,  # This is low because Neel uses L2, and I think we should use mean squared error
        "l1_lambda": 8e-5,
        ...
        "test_every": 500,
        "save_state_dict_every": lambda step: step%9000 == 1, # Disabled currently; used Mod 1 so this still saves immediately. Plus doesn't interfere with resampling (very often)
        "wandb_group": None,
        ...
        "anthropic_resample_batches": 60_000, # 200_000 # 32_000 // 100, # How many batches to go through when doing Anthropic reinit. Should be >=d_sae so there are always enough. Plus 
    ...
    cur_dict["act_name"] = f"blocks.{cur_dict['layer']}.mlp.hook_post"
