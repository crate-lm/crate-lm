...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.query_hook = nn.Identity()
        self.y_hook = nn.Identity()
        # output projection
...
class MLP(nn.Module):
    ...
    def forward(self, x, enhanced_feature_id=None):
        x = self.c_fc(x)
        x = self.relu(x)
        if enhanced_feature_id is not None:
            # print("previous mean value: ", torch.mean(x[:, :, enhanced_feature_id[0]]).item())
            x[:, :, enhanced_feature_id[0]] = enhanced_feature_id[1]
        x = self.c_proj(x)
        x = self.dropout(x)
        return x
...
class Block(nn.Module):
    ...
    def forward(self, x, enhanced_feature_id=None):
        ...
        x = x + self.mlp(self.ln_2(x), enhanced_feature_id=enhanced_feature_id)
        return x
...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type, override_args=None):
        ...
        config_args = {
            'gpt2-small':   dict(n_layer=12, n_head=8, n_embd=512),  # 124M params
