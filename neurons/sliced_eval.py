...
parser.add_argument('--slice_id', type=int, help='slice index to evaluate')
...
async def run():
    ...
    # neuron_idx = 3
    slice_amount = ((768 * 4) * 14 // 14) // 14
    slice_start = slice_id * slice_amount + already_done
    slice_end = (slice_id + 1) * slice_amount
    for neuron_idx in tqdm(range(slice_start, slice_end)):
