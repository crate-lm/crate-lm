...
class MLP(nn.Module):
    def __init__(self, cfg: Union[Dict, HookedTransformerConfig]):
        ...
        self.weight = nn.Parameter(torch.Tensor(4 * cfg.d_model, cfg.d_model))
        self.relu = nn.ReLU()
        with torch.no_grad():
        ...
        self.hook_checkpoint = HookPoint() 
    ...
    def forward(
        ...
        pre_act = self.hook_pre(z_init + grad_update)

        # output_sparse_code = F.relu(z_init + grad_update)
        output_sparse_code = self.relu(pre_act)
        
        post_act = self.hook_post(output_sparse_code)
        
        # output = F.linear(output_sparse_code, self.weight.t(), bias=None)
        output = F.linear(output_sparse_code, self.weight.t(), bias=None)
        checkpoint = self.hook_checkpoint(output)
