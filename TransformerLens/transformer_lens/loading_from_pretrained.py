...
def convert_hf_model_config(model_name: str, **kwargs):
    ...
    elif "crate" in official_model_name.lower():
        architecture = "CrateForCausalLM"
        print("architecture crate")
    else:
    ...
    elif architecture == "CrateForCausalLM":
        if "6l" in official_model_name.lower() or "12l" in official_model_name.lower():
            d_crate_model = 768
            d_crate_head = 768 // 12
            n_crate_heads = 12
            d_crate_mlp = 3072
            ...
            d_crate_mlp = 512
        cfg_dict = {
            "n_layers": int(official_model_name.split("-")[1].split("l")[0]),
            "d_model": d_crate_model,
            "d_head": d_crate_head,
            "n_heads": n_crate_heads,
            "d_mlp": d_crate_mlp,
            "d_vocab": 50304,
            "n_ctx": 1024,
            "dtype": torch.float32,
...
def get_pretrained_state_dict(
    ...
    **kwargs,
    ...
    if (
        official_model_name.startswith("NeelNanda")
        or official_model_name.startswith("crate")
        or official_model_name.startswith("ArthurConmy")
        or official_model_name.startswith("Baidicoot")
    ):
        print("entering crate")
        api = HfApi()
        ...
        device = "cuda"
        print("loading model", official_model_name)
        
        # HYPERPARAMS
        RANDOM_INIT = False
        if official_model_name.endswith("12l"):
            model_url = f'/home/ubuntu/nanogpt4crate/out/ckpt-{official_model_name}-overparam-6000.pt'
        else:
            model_url = f'/home/ubuntu/nanogpt4crate/out/ckpt-{official_model_name}-overparam.pt'
        ...
        if RANDOM_INIT:
            for k, v in checkpoint['model'].items():
                # use randn
                if 'weight' in k:
                    checkpoint['model'][k] = torch.randn(v.shape)
        ...
        elif cfg.original_architecture == "CrateForCausalLM":
            print("entering nanogpt")
            state_dict = convert_nanogpt_weights(state_dict, cfg)
        return state_dict
    else:
        if cfg.from_checkpoint:
            if official_model_name.startswith("stanford-crfm"):
