...
class HookedTransformer(HookedRootModule):
        ...
        if tokenizer is not None:
            ...
            if self.cfg.tokenizer_name in NON_HF_HOSTED_MODEL_NAMES:
                ...
                if "phi" in self.cfg.tokenizer_name.lower():
                    use_fast = False
                print(self.cfg)
                if 'crate' in self.cfg.tokenizer_name:
    ...
    def forward(
        ...
        with utils.LocallyOverridenDefaults(
            ...
            for i, block in blocks_and_idxs[start_at_layer:stop_at_layer]:  # type: ignore
                ...
                # print("Devices: ", devices.get_device_for_block_index(i, self.cfg))
    ...
    def center_writing_weights(self, state_dict: Dict[str, torch.Tensor]):
        ...
        """
        # we don't need to center weights
    ...
    def fold_value_biases(self, state_dict: Dict[str, torch.Tensor]):
        ...
        for layer in range(self.cfg.n_layers):
            ...
            b_V = b_V.to(devices.get_device_for_block_index(0, self.cfg))
            # [head_index, d_head, d_model]
            W_O = state_dict[f"blocks.{layer}.attn.W_O"].to(devices.get_device_for_block_index(0, self.cfg))
            # [d_model]
            b_O_original = state_dict[f"blocks.{layer}.attn.b_O"].to(devices.get_device_for_block_index(0, self.cfg))
