...
class NeuronRecord(FastDataclass):
    ...
    def train_activation_records(
        ...
        """
        # quantile_samples = []
        # for quantile in self.random_sample_by_quantile:
        #     quantile_samples.extend(quantile[0:2])
        # return self.most_positive_activation_records[0:15] + quantile_samples + self.random_sample[0:5]
        return self.most_positive_activation_records[slice(0, 10, 2)]
    ...
    def valid_activation_records(
        ...
        # quantile_samples = []
        # for quantile in self.random_sample_by_quantile:
        #     quantile_samples.extend(quantile[2:4])
        # out_of_context_samples = self.most_positive_activation_records[25:35]
        # for i in range(len(out_of_context_samples)):
        #     out_of_context_samples[i].tokens = out_of_context_samples[i].tokens[3:6]
        #     out_of_context_samples[i].activations = out_of_context_samples[i].activations[3:6]
        # return self.most_positive_activation_records[15:25] + quantile_samples + self.random_sample[5:10] + out_of_context_samples
        return self.random_sample[slice(0, 10, 2)] + self.most_positive_activation_records[slice(1, 10, 2)]
...
def load_neuron(
    ...
    path = bf.join(dataset_path, str(layer_index), f"{neuron_index}.json")
