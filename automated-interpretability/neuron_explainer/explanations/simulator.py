...
def was_token_split(current_token: str, response_tokens: Sequence[str], start_index: int) -> bool:
    ...
    # print("merged_response_tokens:", merged_response_tokens)
...
def parse_simulation_response(
    ...
    # print("text[scoring_start]:", text[scoring_start])
    expected_values = []
    ...
    for i in range(2, len(response_tokens)):
        ...
        # print("len(original_sequence_tokens):", len(original_sequence_tokens))
        ...
        if token_text_offset[i] >= scoring_start:
            ...
            if response_tokens[i - 1] == "\t":
                ...
                if response_tokens[i] != "unknown":
                    logging.warn("Ignoring tab token that is not followed by an 'unknown' token.")
                    continue
                
                # i represents the 'unknown' token
                # j represents the index of the token in a "token<tab>unknown" line, barring
                ...
                # print("response_tokens[i]:", response_tokens[i])
                # print("response_tokens[j]:", response_tokens[j])
                # print("response_tokens[j-5:j+5]:", response_tokens[j-5:j+5])
