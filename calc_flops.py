...
# Configuration
config = CRATEConfig(block_size=1024, vocab_size=50304, n_layer=12, n_head=12, n_embd=768, dropout=0.0, bias=True)
model = CRATE(config)

# FLOP Calculator
flop_calculator = FLOP_Calculator(model, config)
flops = flop_calculator.calculate_flops()

print(f"Estimated FLOPs per token: {flops}")

# Example usage to estimate MFU
fwdbwd_per_iter = 1  # example value
dt = 1  # example time in seconds per iteration
