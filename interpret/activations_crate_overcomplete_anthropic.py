...
def load_model():
    ...
    if MODEL == 'crate-1L':
        ...
        model = CRATE(crateconf)
        print("loading CRATE checkpoint")
    elif MODEL == 'crate-12L':
        ckpt_path = os.path.join(out_dir, 'ckpt-crate-12l-overparam-3000.pt')
        checkpoint = torch.load(ckpt_path, map_location=device)
        crateconf = CRATEConfig(**checkpoint['model_args'])
        model = CRATE(crateconf)
        print("loading CRATE checkpoint")
...
if SAE:
    ...
    cfg = {
        ...
        "d_in": 768 * 4,  # Input dimension for the encoder model
        "d_sae": 768 * 4 * int(SAE.split("x")[0]),  # Dimensionality for the sparse autoencoder (SAE)
...
def get_acts(block_name, layer_id):
    ...
    quantiles = [0.        , 0.08333333, 0.16666667, 0.25      , 0.33333333,        0.41666667, 0.5       , 0.58333333, 0.66666667, 0.75      ,        0.83333333, 0.91666667]
...
def save_json(layer_id, neuron_id, \
    ...
    # print(quantiles)
    neurons['quantile_boundaries'] = quantiles
    neurons['quantile_boundaries'][0] = None
    neurons['quantile_boundaries'][-1] = None
    ...
    path = f"/home/ubuntu/nanogpt4crate/neurons/{block_name}_anthropic/{MODEL_PATH}/{layer_id}"
...
# make a df
# for layer_id in range(int(MODEL[-2])):
